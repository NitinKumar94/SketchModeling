...
def load_data(config, views, shape_list, shuffle=True, batch_size=-1):
    ...
    if config.test:
        ...
        sketch_variation_queue = tf.train.string_input_producer(['%d' % v for v in range(config.sketch_variations)],
                                                                shuffle=True)
    ...
    if config.continuous_view and config.test:
        shape_list_queue = tf.train.input_producer([name for name in shape_list for view in range(num_target_views)],
                                                   shuffle=False)
    else:
    ...
    source_files = [
        config.data_dir + source_prefix_list[view] + shape_name + source_interfix_list[view] + source_suffix_list[view]
        for view in range(num_source_views)]
    if not config.continuous_view:
        target_files = [
            config.data_dir + target_prefix_list[view] + shape_name + target_interfix_list[view] + target_suffix_list[
                view] for view in range(num_target_views)]
        target_angles = tf.zeros([num_target_views, 4])
    else:
        angle_list = [vw.view2angle(view) for view in views.views]
        view_list_queue = tf.train.slice_input_producer(
            [angle_list, target_prefix_list, target_interfix_list, target_suffix_list], shuffle=(not config.test))
        target_files = [config.data_dir + view_list_queue[1] + shape_name + view_list_queue[2] + view_list_queue[
            3]]  # only one single image
    ...
    source_image = image.normalize_image(tf.slice(source_image, [0, 0, 0], [config.image_size, config.image_size,
                                                                            -1]))  # just do a useless slicing to establish size
    source_image = tf.concat([source_image, tf.image.flip_left_right(source_image)],
                             2)  # HACK: add horizontally flipped image as input

    # decode target images

    if not config.test:
        target_images = tf.stack(
            [tf.image.decode_png(tf.read_file(file), channels=4, dtype=tf.uint16) for file in target_files])
        target_images = image.normalize_image(
            tf.slice(target_images, [0, 0, 0, 0], [-1, config.image_size, config.image_size, -1]))
    else:
        target_images = tf.ones(
            [len(target_files), config.image_size, config.image_size, 4])  # dummy target for testing
    ...
    if config.predict_normal:
        ...
        target_background = tf.concat([tf.zeros(target_shape[:-1] + [2]), tf.ones(target_shape[:-1] + [2])],
                                      3)  # (0,0,1,1)
