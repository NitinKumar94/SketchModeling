...
class MonNet(object):
    ...
    def build_network(self, names, sources, targets, masks, angles, views, is_training=False, is_validation=False,
                      is_testing=False, is_encoding=False):
        """
            input:
        ...
        if not self.config.continuous_view:
            with tf.variable_scope(var_scope_G):
                with tf_framework.arg_scope(layer.unet_scopes(bn_scope_G)):
                    preds, features = network.generateUNet(sources, num_output_views,
        ...
        sources_expanded = tf.reshape(tf.tile(sources, [1, num_output_views, 1, 1]),
                                      [-1, source_size[1], source_size[2], source_size[3]])  # (n*m) x H x W x C
        ...
        learning_rate = tf.train.exponential_decay(init_learning_rate, global_step=opt_step, decay_steps=10000,
        ...
        self.grad_G_placeholder = [(tf.placeholder(tf.float32, shape=grad[1].get_shape()), grad[1]) for grad in grad_G
                                   if grad[0] is not None]
        self.grad_G_list = [grad[0] for grad in grad_G if grad[0] is not None]
        self.update_G_op = opt_G.apply_gradients(self.grad_G_placeholder,
        ...
        if not self.config.no_adversarial:
            ...
            self.grad_D_placeholder = [(tf.placeholder(tf.float32, shape=grad[1].get_shape()), grad[1]) for grad in
                                       grad_D if grad[0] is not None]
        ...
        while True:
            ...
            if do_print:
                ...
                log_str_2 = 'losses: %7.3g, %7.3g, %7.3g, %7.3g, %7.3g, %7.3g;' % \
                            (step_losses_G[0], step_losses_G[1], step_losses_G[2], step_losses_D[0], step_losses_D[1],
                             step_losses_D[2])
                print(log_str_1, end='')
                print(log_str_2)
    ...
    def test(self, sess, views, num_shapes):
        ...
        while not finished:
            ...
            for k in range(len(names)):
                ...
                name_gt = os.path.join(self.config.test_dir, output_images_folder, shape_name,
                                       ('gt-' + output_prefix + '--' + view_name + '.png'))
                name_output = os.path.join(self.config.test_dir, output_images_folder, shape_name,
                                           ('pred-' + output_prefix + '--' + view_name + '.png'))
                image.write_image(name_gt, images[1, k])
                image.write_image(name_output, images[2, k])

                name_normal = os.path.join(self.config.test_dir, output_images_folder, shape_name,
                                           ('normal-' + output_prefix + '--' + view_name + '.png'))
                name_depth = os.path.join(self.config.test_dir, output_images_folder, shape_name,
                                          ('depth-' + output_prefix + '--' + view_name + '.png'))
                name_mask = os.path.join(self.config.test_dir, output_images_folder, shape_name,
                ...
                # export results
                name_output = os.path.join(self.config.test_dir, output_results_folder, shape_name,
    ...
    def validate_output(self, sess, num_shapes, epochs):
        ...
        for k in range(len(names)):
            ...
            name_output = os.path.join(self.config.train_dir, valid_results_folder, shape_name,
                                       ('output--' + view_name + '.png'))
            name_gt = os.path.join(self.config.train_dir, valid_results_folder, shape_name,
                                   ('gt--' + view_name + '.png'))
            image.write_image(name_output, images[0, k])
            image.write_image(name_gt, images[1, k])

            name_normal = os.path.join(self.config.train_dir, valid_results_folder, shape_name,
                                       ('normal--' + view_name + '.png'))
            name_depth = os.path.join(self.config.train_dir, valid_results_folder, shape_name,
                                      ('depth--' + view_name + '.png'))
            name_mask = os.path.join(self.config.train_dir, valid_results_folder, shape_name,
                                     ('mask--' + view_name + '.png'))
