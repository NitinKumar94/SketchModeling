...
def compute_mask_loss(predicts, targets, normalized=True):
    ...
    mask_loss = tf.reduce_sum(
        -tf.multiply(tf.log(tf.maximum(1e-6, p)), z) - tf.multiply(tf.log(tf.maximum(1e-6, 1 - p)), 1 - z))
...
def compute_corres_geom_loss(predicts, corres, views):
    ...
    for batch_id in range(num_batches):
        ...
        slice_points = tf.reshape(tf.gather(all_points, all_corres),
                                  [views.num_edges, -1, views.edge_size, 3])  # G x M x v x 3
        slice_dirs = tf.reshape(tf.gather(all_dirs, all_corres),
                                [views.num_edges, num_samples, views.edge_size, 3])  # G x M x v x 3

        # compute position loss as variance of reprojected point positions across nearby views
        normalized_points = slice_points - tf.tile(tf.reduce_mean(slice_points, reduction_indices=2, keep_dims=True),
                                                   [1, 1, views.edge_size, 1])  # G x M x v x 3
        ...
        lensq_dirs = tf.maximum(tf.reduce_sum(tf.multiply(slice_dirs, slice_dirs), reduction_indices=3, keep_dims=True),
                                1e-3)
        ...
        direction_loss = 1.0 - tf.reduce_mean(tf.matmul(transposed, transposed, transpose_b=True)) * (
        1.0 / (views.num_edges * num_samples))
...
def compute_corres_mask_loss(predicts, corres, views):
    ...
    for batch_id in range(num_batches):
        ...
        all_corres = tf.reshape(batch_corres[batch_id], [-1])  # (G*M*v)
        slice_probs = tf.reshape(tf.gather(all_probs, all_corres),
                                 [views.num_edges, -1, views.edge_size, 1])  # G x M x v x 1

        # compute mask loss as Jensen-Shannon divergence of predicted mask probabilities across nearby views
        mask_loss = tf.reduce_mean(compute_entropy(tf.reduce_mean(slice_probs, reduction_indices=1)) - tf.reduce_mean(
            compute_entropy(slice_probs), reduction_indices=1))
