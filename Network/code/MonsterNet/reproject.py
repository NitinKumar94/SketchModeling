...
def transform_tensor(predicts, views):
    ...
    predicts_per_view = tf.transpose(tf.reshape(predicts, [-1, num_views, shape[1], shape[2], shape[3]]),
                                     [1, 0, 2, 3, 4])  # V x n x H x W x 4
    depths_per_view = tf.unstack(
        tf.slice(predicts_per_view, [0, 0, 0, 0, 3], [-1, -1, -1, -1, 1]))  # [n x H x W x 1] * V
    normals_per_view = tf.unstack(
        tf.slice(predicts_per_view, [0, 0, 0, 0, 0], [-1, -1, -1, -1, 3]))  # [n x H x W x 3] * V
    ...
    homo_points_per_view = [tf.concat([mat_u, mat_v, mat_d, mat_w], 3) for mat_d in
                            depths_per_view]  # [n x H x W x 4] * V
    ...
    for view_id in range(num_views):
        xformed = tf.matmul(tf.reshape(homo_points_per_view[view_id], [-1, 4]), xform_per_view[view_id],
                            transpose_b=True)  # (n*H*W) x 4
        rotated = tf.matmul(tf.reshape(homo_dirs_per_view[view_id], [-1, 4]), rotate_per_view[view_id],
                            transpose_b=True)  # (n*H*W) x 4
        points_per_view[view_id] = tf.slice(tf.reshape(xformed, [-1, H, W, 4]), [0, 0, 0, 0],
                                            [-1, -1, -1, 3])  # n x H x W x 3
        dirs_per_view[view_id] = tf.slice(tf.reshape(rotated, [-1, H, W, 4]), [0, 0, 0, 0],
                                          [-1, -1, -1, 3])  # n x H x W x 3
